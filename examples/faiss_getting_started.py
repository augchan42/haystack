...
from haystack.utils.faiss_getting_started import build_pipeline, add_example_data
    ...
    if os.path.exists(faiss_index_path):
        ...
        try:
            ...
            if document_store.get_document_count() == 0 or not document_store.is_index_correctly_built():
                reindex_document_store(document_store, 
                                       embedding_retriever, 
                                       faiss_index_path, 
                                       faiss_config_path)
...
def create_new_faiss_doc_store():
    document_store = FAISSDocumentStore(
        sql_url="sqlite:///my_faiss_document_store.db",
...
def getting_started(provider, API_KEY, API_BASE: Optional[str] = None):
    ...
    embedding_retriever = EmbeddingRetriever(
        document_store=None,  # We'll set the document_store later
    ...
    # logger.info("Saving the updated FAISS index")
    # document_store.save(index_path=faiss_index_path, config_path=faiss_config_path)
